...
const NAV = [
  ...
  { id:'conferencia', icon:IconScale,       label:'Conferência' },
]
...
export default function Sidebar({ active, onChange }) {
  ...
  return (
    <aside className="w-[248px] flex-shrink-0 flex flex-col h-screen sticky top-0 border-r"
      style={{ background:'#131313', borderColor:'rgba(255,255,255,0.06)' }}>

      <div className="px-6 py-7 flex items-center gap-3">
        {LOGO}
        <div className="w-px h-8" style={{ background:'rgba(255,255,255,0.1)' }} />
        <p className="text-[11px] font-bold tracking-[1.5px] uppercase leading-tight" style={{ color:'rgba(255,255,255,0.4)' }}>
          Painel de<br/>Administração
        </p>
      </div>

      <nav className="flex-1 px-3.5 pt-2 flex flex-col gap-0.5 overflow-y-auto">
        {NAV.map(item => {
          const isActive = active === item.id
          const Icon = item.icon
          return (
            <button key={item.id} onClick={() => onChange(item.id)}
              className="relative flex items-center gap-3 pl-4 pr-3 py-2.5 rounded-lg text-[13.5px] font-semibold text-left w-full transition-colors"
              style={{
                color: isActive ? '#fff' : 'rgba(255,255,255,0.45)',
                background: isActive ? 'rgba(232,0,13,0.1)' : 'transparent',
              }}
            >
              {isActive && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-4 rounded-full" style={{ background:'#E8000D' }} />
              )}
      ...
      <div className="p-3.5 flex flex-col gap-1.5 border-t" style={{ borderColor:'rgba(255,255,255,0.06)' }}>
        <a href="/"
          className="flex items-center gap-2.5 pl-4 pr-3 py-2.5 rounded-lg text-[13px] font-semibold transition-colors"
        ...
        <button onClick={logout}
          className="flex items-center gap-2.5 pl-4 pr-3 py-2.5 rounded-lg text-[13px] font-semibold transition-colors text-left"
          style={{ color:'rgba(255,255,255,0.45)' }}
          onMouseEnter={e => e.currentTarget.style.color = '#FF4550'}
          onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.45)'}
        >
          <IconLogout size={16} />
          Sair da Sessão
