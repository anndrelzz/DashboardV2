...
import { IconPerformance } from './icons'
...
export default function SecPerformance({ equipes, vendedores, vendas }) {
  ...
  return (
    <div className="p-10 flex flex-col gap-8">
      <SectionHeader icon={<IconPerformance size={20} />} title="Desempenho por Equipe" subtitle="Ranking e produtividade dos membros" />
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ background:'#161616', borderColor:'rgba(255,255,255,0.07)' }}>
        ...
        <div className="p-6">
          <select value={eqId} onChange={e => setEqId(e.target.value)}
            className="rounded-lg px-4 py-2.5 text-white text-sm outline-none border border-white/10 focus:border-red/40 w-80"
        ...
        <>
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ background:'#161616', borderColor:'rgba(255,255,255,0.07)' }}>
            ...
            <table className="w-full">
              ...
              <tbody>
                {ranking.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-14 text-muted text-[13px] tracking-wider uppercase">Nenhum membro nesta equipe</td></tr>
                ) : ranking.map((v, i) => (
                  <tr key={v.id} className="border-b last:border-0 hover:bg-white/[0.02] transition-colors"
                    style={{ borderColor:'rgba(255,255,255,0.05)' }}>
                    <td className="px-6 py-3.5 text-[13.5px] font-bold text-muted">{i+1}º</td>
                    <td className="px-6 py-3.5 text-[13.5px] font-semibold">{v.nome}</td>
                    <td className="px-6 py-3.5">
                      {v.nivel ? <span className="text-[12px] font-bold" style={{ color:NIVEL_CORES[v.nivel]||'#fff' }}>{v.nivel}</span> : '—'}
                    </td>
                    <td className="px-6 py-3.5 text-[13.5px] text-muted">{v.qtd}</td>
                    <td className="px-6 py-3.5 font-bebas text-lg text-red">{fmt(v.total)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
